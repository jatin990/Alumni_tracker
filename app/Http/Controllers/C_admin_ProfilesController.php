...
class C_admin_ProfilesController extends Controller
{
    ...
    public function index(\App\C_admin $c_admin)
    {
        ...
        $unverified_alumni = \App\Profile::where([['verified', 0], ['rejected', 0], ['college', $c_admin->college]])->orderBy('user_id', 'desc')->join('users', 'profiles.user_id', '=', 'users.id')->paginate(4);
